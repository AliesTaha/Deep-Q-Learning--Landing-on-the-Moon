...
def test_network(target):
    ...
    state_size = 8
    i = 0
    assert len(target.layers) == 3, f"Wrong number of layers. Expected 3 but got {
        len(target.layers)}"
    assert target.input.shape.as_list() == [None, state_size], \
        f"Wrong input shape. Expected [None,  400] but got {
...
def test_optimizer(target, ALPHA):
    assert type(target) == Adam, f"Wrong optimizer. Expected: {
        Adam}, got: {target}"
    assert np.isclose(target.learning_rate.numpy(), ALPHA), f"Wrong alpha. Expected: {
        ALPHA}, got: {target.learning_rate.numpy()}"
    print("\033[92mAll tests passed!")
...
def test_compute_loss(target):
    ...
    assert np.isclose(loss, 0.6991737), f"Wrong value. Expected {
        0.6991737}, got {loss}"
    ...
    assert np.isclose(loss, 0.343270182), f"Wrong value. Expected {
        0.343270182}, got {loss}"
